<template>
  <div>
    <form action="#" class="card card-body" @submit.prevent="formSubmit">
      <h5 class="card-title form__title text-center">Зарегистрироваться в <span class="form__title--red">ZenMoney</span></h5>
      <div class="form-group">
        <label for="name">Имя</label>
        <input type="text" class="form-control" :class="{ 'is-invalid': $v.name.$error }" id="name" v-model.trim="name">
    ...
    <div class="text-center mt-3">
      <router-link tag="a" :to="{ name: 'login' }" class="link">Уже есть аккаунт?</router-link>
    </div>
  </div>
</template>
